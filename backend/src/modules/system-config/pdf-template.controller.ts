...
export class PdfTemplateController {
  ...
  @Get()
  @Roles("MANAGER")
  async getPdfTemplate(@Req() req: Request): Promise<PdfHtmlTemplate> {
    return this.systemConfigService.getPdfHtmlTemplate(req.user?.branchId);
  }

  @Put()
  @Roles("MANAGER")
  async updatePdfTemplate(@Body() template: PdfHtmlTemplate, @Req() req: Request): Promise<PdfHtmlTemplate> {
